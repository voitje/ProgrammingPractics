...
namespace lab3
{
	...
	char* ReplaceTabsOnSpaces(char* string)
	{
		...
		for (int i = 0; i < strlen(string); i++, j++)
		{
			string1[j] = string[i];
			if (string[i] == '\t')
			{//TODO: ¬сЄ ещЄ жЄстко прибито, что 4 пробела, мен€ютс€ на табы.
				if (j % 4 == 0)
				{	
					int temp = j;
					string1[j] = ':';
				...
				{//TODO: ¬сЄ ещЄ жЄстко прибито, что 4 пробела, мен€ютс€ на табы. \ в else не 4 пробела
					while (j % 4 != 0)
					{
						string1[j] = ':';
						j++;
	...
	lab3::Person ReadPerson()
	{
		...
		cout << "\nPlease enter name and surname (ex. УJack BauerФ):\n>";
	...
	void MenuThirdLaboratory()
	{
		...
		while (asciiValue != escapeSymbol)
		{
			...
			asciiValue = key;
			switch (asciiValue)
			{//TODO: ѕосле некоторых манипул€ций - лаба перестала работать. ќттестируйте лучше!
				case '1':
				{
					while (asciiValue != escapeSymbol)
					{
